...
describeNonPool("StakingRewards", async () => {
  ...
  describe("Rewards Initialization/Termination", async () => {
    ...
    it("verifies rewards can be terminated if no funds are staked", async () => {
        ...
        const balanceBefore = await rewardsToken.contract.balanceOf(owner.address);
        await stakingRewards.terminate();
        const balanceAfter = await rewardsToken.contract.balanceOf(owner.address);

        expect(balanceAfter).to.equal(balanceBefore.add(toWei(rewards)));
    });
  ...
  describe("Fees", async () => {
    it("verifies that the Effective Early Withdrawal Fee is applied to the earned rewards when rewards are claimed", async () => {
        ...
        const rewardTokenBalanceBefore = await rewardsToken.contract.balanceOf(user.address);
        await stakingRewards.claimRewardsTo(tokenId, user.address);
        const rewardsClaimedIncludingFees = (await rewardsToken.contract.balanceOf(user.address)).sub(rewardTokenBalanceBefore);
        ...
        const error = Math.abs(rewardsClaimedIncludingFees / expectedRewardsClaimedIncludingFees - 1);
  ...
  describe("Math", async () => {
    ...
    for (let i = 0; i < testCases.length; i++) {
        ...
        it(`rewards calculation case #${i + 1}`, async () =>
        {
            ...
            const error = totalRewards.div(totalRewards.toDecimal(expectedTotalRewards)).sub(1).abs();
